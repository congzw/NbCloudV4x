...
namespace NbCloud.Common.Ioc
{
    ...
    public abstract class ServiceLocatorImplBase : IServiceLocator
    {
        ...
        {
            return string.Format(CultureInfo.CurrentUICulture, ActivationExceptionMessage, serviceType.Name, key);
        }
        ...
        {
            return string.Format(CultureInfo.CurrentUICulture, ActivateAllExceptionMessage, serviceType.Name);
        }

        public abstract bool IsRegistered(Type type);
        public abstract bool IsRegistered<T>();

        public const string ActivateAllExceptionMessage = "Activation error occured while trying to get all instances of type {0}";
        public const string ActivationExceptionMessage = "Activation error occured while trying to get instance of type {0}, key {1}";
    }
}
