...
namespace NbCloud.Web
{
    public class NinjectConfig
    {
        ...
        public static void LoadNinjectModules(IKernel kernel, bool throwEx = true)
        {
            var assemblies = LoadAssembly();
            ...
            //FixMultiBindings<RouteCollection>(kernel);
            //FixMultiBindings<HttpContextBase>(kernel);
            //FixMultiBindings<HttpContext>(kernel);
            //FixMultiBindings<IFilterProvider>(kernel);
        }


        private static void FixMultiBindings<T>(IKernel kernel)
        {
            ...
            if (bindings.Count > 1)
            {
                ...
                {
                    kernel.RemoveBinding(bindings[i]);
                }
            }
        ...
        private static IList<Assembly> LoadAssembly()
        {
